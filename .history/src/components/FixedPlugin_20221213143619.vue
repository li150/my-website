<template>
  <div class="right-plugin">
    <div v-show="!isZh" @click="clickItem(ind)" class="item flex hide" v-for="(item, ind) of list" :key="ind">
      <img :src="item.icon" class="item-icon" />
      <div class="hide">{{ item.name }}</div>
    </div>

    <div v-if="useData.$state.clientWidth > useData.$state.maxWidth && isZh">
      <el-popover placement="left-start" :width="220" trigger="click" v-for="(item, ind) of list" :key="ind + 'ss'">
        <template #reference>
          <div class="item flex hide">
            <img :src="item.icon" class="item-icon" />
            <div class="hide">{{ item.name }}</div>
          </div>
        </template>
    ...
    <div class="item" v-if="useData.$state.clientWidth < useData.$state.maxWidth && !isZh">
      <picture>
        <source srcset="../assets/home/mobile-call.png" media="(min-width:992px)" />
        <img @click="clickItem(0)" src="../assets/home/whatsapp@2x.png" class="item-icon" />
      </picture>
    </div>
    ...
    <div class="item" v-if="useData.$state.clientWidth < useData.$state.maxWidth && isZh">
      <el-popover placement="left-start" :width="220" trigger="click">
        ...
        <img src="../assets/home/wechat-qrcode.png" style="width: 200px; height: 200px; display: block" />
      </el-popover>
    </div>
    <div @click="clickItem(3)" class="item flex top" v-show="isShowGoTop" :style="isZh ? 'margin:0' : ''">
